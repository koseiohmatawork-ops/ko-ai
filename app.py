...
def simple_save_final_post_from_result_next_post(file_path: Path, platform: str) -> Path:
    ...
    # 同じ次投稿案・同じ投稿先の完成版がすでにある場合は、新規作成せず既存ファイルを使う。
    for existing_final_path in sorted(save_dir.glob("*.md"), reverse=True):
        existing_content = existing_final_path.read_text(encoding="utf-8")
        existing_source = simple_extract_field(existing_content, "元ファイル")
        existing_platform = simple_extract_field(existing_content, "投稿先")
        if existing_source == str(file_path) and existing_platform.lower() == platform.lower():
            return existing_final_path
...
def simple_save_scheduled_post_from_final_post(file_path: Path, status: str = "今日投稿") -> Path:
    ...
    # 同じ完成版・同じ状態の投稿予定がすでにある場合は、新規作成せず既存ファイルを使う。
    for existing_schedule_path in sorted(schedule_dir.glob("*.md"), reverse=True):
        existing_content = existing_schedule_path.read_text(encoding="utf-8")
        existing_source = simple_extract_field(existing_content, "元ファイル")
        existing_status = simple_extract_field(existing_content, "状態")
        if existing_source == str(file_path) and existing_status == status:
            return existing_schedule_path
