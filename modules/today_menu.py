...
投稿ストック整理案がある場合は、今後も使うべき投稿案・優先テーマ・archive候補を参考にしてください。
安全チェック済み投稿がある場合は、危ない表現を修正した安全な投稿文として最優先で参考にしてください。安全チェック済み投稿と未チェック投稿が矛盾する場合は、必ず安全チェック済み投稿の表現を採用してください。
完成版投稿がある場合は、実際に投稿できる最終版として最優先で参考にしてください。
分析実投稿がある場合は、次に投稿すべき具体案として最優先で参考にしてください。
投稿反応から作った次の投稿案がある場合は、実際の反応をもとにした次回投稿候補として最優先で参考にしてください。
...
実在しない成功事例や実績を断定しないでください。実績がない場合は必ず「架空例」「シミュレーション」「例」と明記してください。「〇〇さんが月5万円を実現」のような実在の実績に見える表現は禁止です。
「誰でも稼げる」「必ず月◯万円」「月◯万円を実現」「副収入を実現」など、収益を保証・断定する表現は避けてください。収益に触れる場合は「目指す」「可能性がある」「一例」と弱めてください。
「私もよくそうでした」「初心者の私でも」など、本人の実体験に見える表現は避けてください。
「〇〇法」「〇〇するだけ」など、未完成・不自然・過度に簡単そうな表現は避けてください。
「リスクが少ない」「安全に始められる」など、損失や安全性を断定する表現は避けてください。
...
投稿ストック整理案がある場合は、今後も使うべき投稿案・優先テーマ・archive候補を分析に反映してください。
安全チェック済み投稿がある場合は、修正後の安全な投稿文を高く評価し、危ない表現を避ける参考にしてください。
完成版投稿がある場合は、今後も使える投稿の型として高く評価してください。
分析実投稿がある場合は、どの投稿案がすぐ投稿できて収益導線に近いかも評価してください。
投稿反応から作った次の投稿案がある場合は、反応データを踏まえた改善候補として高く評価してください。
テンプレ投稿がある場合は、どの型が収益導線に使いやすいかも評価してください。
著作権・性的表現・炎上リスクが高い二次創作系テーマは低評価にしてください。
実在しない成功事例や実績を断定する投稿は低評価にしてください。実績がない場合は「架空例」「シミュレーション」「例」と明記する方向にしてください。
「誰でも稼げる」「必ず月◯万円」など、収益を保証する表現は避ける方向にしてください。
「私もよくそうでした」「初心者の私でも」など、本人の実体験に見える表現は避けてください。
「〇〇法」「〇〇するだけ」など、未完成・不自然・過度に簡単そうな表現は避けてください。
「リスクが少ない」「安全に始められる」など、損失や安全性を断定する表現は避けてください。
...
・分析実投稿がある場合は、今後も使うべき投稿案と改善すべき投稿案を分けて整理する
・投稿反応から作った次の投稿案がある場合は、今後優先して使うべき候補として整理する
・安全チェック済み投稿がある場合は、修正後の安全な投稿文を今後使うべき投稿として評価する
・完成版投稿がある場合は、今後も使うべき最終投稿ストックとして整理する
・実在しない成功事例や実績を断定している投稿は、表現を修正するか優先度を下げる
...
4. 分析実投稿の中で今後も使うべき投稿案
5. 優先して投稿に使うべきテーマ
6. archiveに移してもよいストック
7. 使わない方がよい理由
8. 今後作るべき安全な投稿テーマ10個
9. 今日からの運用ルール
